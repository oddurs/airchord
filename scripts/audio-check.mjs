...
try {
  ...
  console.log('\n  articulation\n')
  console.log('  case                      dip  allowed')
  for (const strike of report.strikes ?? []) {
    // Each voice is judged against what it declared, not against the others.
    const bad = strike.dip > strike.ceiling
    if (bad) failures.push(`${strike.label} only dips to ${strike.dip}, and declares ${strike.ceiling}`)
    console.log(
      `  ${strike.label.padEnd(22)} ${String(strike.dip).padStart(6)}   ${String(strike.ceiling).padStart(6)}${bad ? '  ✗' : ''}`,
    )
  }
